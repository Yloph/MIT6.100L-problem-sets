...
def filter( pixels_list , color ):
    ...
    """
    res = []
    trans_matrix = make_matrix( color )
    for r,g,b in pixels_list:

        pixel_vec = [ [ r ] , [ g ] , [ b ] ]
        trans_pixel = matrix_multiply( trans_matrix , pixel_vec )
        new_r = int( round( trans_pixel[ 0 ][ 0 ] ) ) # type: ignore
        new_g = int( round( trans_pixel[ 1 ][ 0 ] ) ) # type: ignore
        new_b = int( round( trans_pixel[ 2 ][ 0 ] ) ) # type: ignore
        res.append( ( new_r , new_g , new_b ) )
...
def extract_end_bits( num_end_bits , pixel ):
    ...
    if type( pixel ) == int:
        ...
        return pixel % pow( 2 , num_end_bits )
    
    if type( pixel ) == tuple:

        new_r = int( round( pixel[ 0 ] % pow( 2 , num_end_bits ) ) )
        new_g = int( round( pixel[ 1 ] % pow( 2 , num_end_bits ) ) )
        new_b = int( round( pixel[ 2 ] % pow( 2 , num_end_bits ) ) )
...
def reveal_bw_image( filename ):
    ...
    for pixel in pixel_list:
        ...
        lsbed_pixel = extract_end_bits( 1 , pixel )
        rescaled_pixel = ( lsbed_pixel * 255 ) // 1 # type: ignore
        res.append( rescaled_pixel )
    
    size = Image.open( filename ).size
    img = pix_to_img( res , size , 'L' )
    img.save("pset5-part2a.bmp")
    return img
...
def reveal_color_image(filename):
    ...
    for pixel in pixel_list:
        ...
        lsbed_r = extract_end_bits( 3 , pixel[ 0 ] )
        lsbed_g = extract_end_bits( 3 , pixel[ 1 ] )
        lsbed_b = extract_end_bits( 3 , pixel[ 2 ] )
        # use floor division !!!
        rescaled_r = ( lsbed_r * 255 ) // 7 # type: ignore
        rescaled_g = ( lsbed_g * 255 ) // 7 # type: ignore
        rescaled_b = ( lsbed_b * 255 ) // 7 # type: ignore
        res_pixel = ( rescaled_r , rescaled_g , rescaled_b )
        res.append( res_pixel )

    size = Image.open( filename ).size
    img = pix_to_img( res , size , "RGB" )
    img.save("pset5-part2b.bmp")
    return img

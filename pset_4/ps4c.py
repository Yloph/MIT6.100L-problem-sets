...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    best_count = -1
    best_pad = []
    dictionary = load_words( WORDLIST_FILENAME )
    for pad in pads:

        plaintext = ciphertext.decrypt_message( pad ).get_text()
        words_list = plaintext.split()
        count = 0
        for word in words_list:

            if is_word( dictionary , word ):
...
def decode_story():
    ...
    story_in_Encryptedtext = ps4b.EncryptedMessage( story_string )
    pads = get_story_pads()
    return decrypt_message_try_pads( story_in_Encryptedtext , pads ).get_text()
